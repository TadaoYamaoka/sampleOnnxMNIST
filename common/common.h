...
inline uint32_t read_big32(std::ifstream& ifs)
{
    uint32_t value;
...
inline int readMNISTFile(const std::string& imgFileName, const std::string& labelFileName, uint8_t* buffer, int inH, int inW)
{
    std::ifstream imgFile(imgFileName, std::ifstream::binary);
    assert(imgFile.is_open() && "Attempting to read from a file that is not open.");
    uint32_t magic = read_big32(imgFile);
    uint32_t max = read_big32(imgFile);
    uint32_t h = read_big32(imgFile);
    uint32_t w = read_big32(imgFile);
    assert(h == inH);
    assert(w == inW);
    int pos = rand() % max;
    imgFile.seekg(pos * inH * inW, imgFile.cur);
    imgFile.read(reinterpret_cast<char*>(buffer), inH * inW);
    ...
    magic = read_big32(labelFile);
    max = read_big32(labelFile);
    labelFile.seekg(pos, labelFile.cur);
    byte label;
