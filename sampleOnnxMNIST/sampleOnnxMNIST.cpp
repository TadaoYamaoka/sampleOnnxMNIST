...
bool SampleOnnxMNIST::build()
{
    ...
    std::cout << "Input name : " << network->getInput(0)->getName() << std::endl;

    assert(network->getNbOutputs() == 1);
    mOutputDims = network->getOutput(0)->getDimensions();
    assert(mOutputDims.nbDims == 2);
    std::cout << "Output name : " << network->getOutput(0)->getName() << std::endl;
...
bool SampleOnnxMNIST::processInput(const samplesCommon::BufferManager& buffers)
{
    ...
    mNumber = readMNISTFile(locateFile("t10k-images-idx3-ubyte", mParams.dataDirs), locateFile("t10k-labels-idx1-ubyte", mParams.dataDirs), fileData.data(), inputH, inputW);
    ...
    for (int i = 0; i < inputH * inputW; i++)
    {
        hostDataBuffer[i] = float((fileData[i] - 0.1307) / 0.3081);
    }
...
samplesCommon::OnnxSampleParams initializeSampleParams(const samplesCommon::Args& args)
{
    ...
    if (args.dataDirs.empty()) //!< Use default directories if user hasn't provided directory paths
    {
        ...
        params.dataDirs.push_back("data/mnist/raw/");
    }
